...
private:
    static bool booted;
    static llama_context *llm_ctx;
    static llama_model *llm_model;
    static bool loadLlmModel(llama_context *&ctx, llama_model *&model);
...
llama_context *Interpreter::llm_ctx = nullptr;
llama_model *Interpreter::llm_model = nullptr;
bool Interpreter::booted = false;

void Interpreter::boot()
{

    if (!Interpreter::booted)
    {
        std::cout << "Iniciando o cérebro" << std::endl;

        llama_backend_init();

        if (!Interpreter::loadLlmModel(Interpreter::llm_ctx, Interpreter::llm_model))
        {
            throw std::runtime_error("Falha ao iniciar o cérebro LLM.");
        }

        Interpreter::booted = true;
    }
}

bool Interpreter::loadLlmModel(llama_context *&ctx, llama_model *&model)
{
    ...
    model = llama_model_load_from_file(LLM_MODEL_PATH.c_str(), model_params);
...
{
    int max_tokens = 128;
    const llama_vocab *vocab = llama_model_get_vocab(Interpreter::llm_model);
    ...
    if (llama_decode(Interpreter::llm_ctx, batch) != 0)
    {
    ...
    float *logits = llama_get_logits_ith(Interpreter::llm_ctx, batch.n_tokens - 1);
    ...
    for (int i = 0; i < max_tokens; ++i)
    {
        ...
        // Pega novos logits
        logits = llama_get_logits_ith(Interpreter::llm_ctx, 0);
